...
public class InvoiceReportService {
    ...
    public ByteArrayInputStream getInvoiceReport(Integer idInvoice) {
        ...
        PDDocument doc = getInvoicePDF(idInvoice);
    ...
    private PDDocument getInvoicePDF(Integer idInvoice) {

        Invoice invoice = invoiceRepository.findById(idInvoice).orElseThrow();
        User user = invoice.getUserIdUser();
        ...
        try {
            ...
            cos.newLineAtOffset(50, rect.getHeight() - 75);
            ...
            cos.newLine();
            cos.setFont(helvetica, 14);
            cos.showText(user.getFirstName() + " " + user.getLastName());
            cos.newLine();
            cos.showText("ul. " + user.getAddress());
            cos.newLine();
            cos.showText(user.getPostalCode() + " " + user.getCity());
            cos.newLine();
            cos.showText(user.getEmail());
            cos.endText();

            // Items table header
            cos.beginText();
            cos.newLineAtOffset(50, rect.getHeight() - 280);
            cos.setFont(helveticaBold, 14);
            ...
            for(PDPage newPage : document.getPages()){
                ...
                newCos.moveTo(50, 50);
                newCos.lineTo(rect.getWidth() - 50, 50);
                newCos.stroke();

                newCos.beginText();
                newCos.newLineAtOffset(50, 40);
                ...
                newCos.newLineAtOffset(rect.getWidth() - 50 - (helvetica.getStringWidth(pageString) / 100), 40);
                newCos.setFont(helvetica, 10);
